...
function weightedPick(voices: SayPersonaId[], counts: Record<string, number>): SayPersonaId {
...
export async function deliverReachOutForUser(
  uid: string,
  timezoneOffset: number,
  db: FirebaseFirestore.Firestore,
): Promise<{ delivered: boolean; personaId?: string }> {
  // Check time window
  ...
  try {
    ...
    const tokenData = tokenSnap.data() as
      | { token?: string; isPremium?: boolean; mutedVoices?: string[] }
      | undefined
